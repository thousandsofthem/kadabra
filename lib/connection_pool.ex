defmodule Kadabra.ConnectionPool do
  ...
  def handle_info({:EXIT, _pid, {:ssl_error, x}}, state) do
    Logger.warning(
      'TLS client: In state connection received SERVER ALERT: Fatal - Certificate Expired? #{inspect x}'
    )

    {:noreply, state}
  end

  def handle_info({:EXIT, _pid, x}, state) do
    Logger.warning(
      'TLS client: In state connection received SERVER ALERT: Fatal - #{inspect x}'
    )
